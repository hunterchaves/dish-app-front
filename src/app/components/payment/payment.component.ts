...
export class PaymentComponent implements OnInit {
  ...
  observation: string = ''; // New property for observation
  ...
  placeOrder(): void {
    this.cartItems$.pipe(
      ...
      switchMap(([items, total]) => {
        ...
        return this.orderService.placeOrder(items, this.selectedPaymentMethod, paymentDetails, this.observation);
      })
    ).subscribe({
      next: (order: any) => {
        alert('Pedido finalizado com sucesso!');
        this.cartService.clearCart();
        this.router.navigate(['/admin/kitchen-orders']);
      },
      error: (err) => {
